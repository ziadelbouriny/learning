...
class RaspberryPiApiClient:
    ...
    def get_latest_trip(self, vin):
    	...
    	try:
            ...
            # Try to extract session_id, trip_id, or latest_session first
            session_id = data.get('starttime')
        
            '''# If not found, try parsing from starttime or other available info
            if not session_id:
                # Example: Use starttime as a fallback session identifier
                session_id = data.get('starttime')'''
...
class DataAnalyzer:
    ...
    def analyze_obd_data(obd_data, speed_analyzer):
        ...
        try:
            ...
            print(df.columns)
            # Determine if trip occurred mostly during night or day
            ...
            # Count harsh acceleration and braking events
            harsh_acceleration_threshold = 2.5  # m/s²
            harsh_braking_threshold = -2.5      # m/s²

            # Detect and record harsh events with location
            harsh_events = []
            ...
            for idx, row in df.iterrows():
                ...
                if acc > harsh_acceleration_threshold:
                    ...
                    continue  # Skip non-harsh events
        ...
        try:
            ...
            if isinstance(meta_data, dict):
                ...
                analysis_result.update({
                    ...
                    "traffic_violations": meta_data.get('traffic_violations', 0)
                })
                
            elif isinstance(meta_data, str):
                # Parse text format
                patterns = {
                    'safe_distractions': r'Safe Distraction.*?(\d+)',
                    'critical_distractions': r'Critical Distraction.*?(\d+)',
                    'lane_violations': r'Lane.*?[Vv]iolation.*?(\d+)',
                    'traffic_violations': r'Traffic.*?[Vv]iolation.*?(\d+)'
                }
                
                for key, pattern in patterns.items():
                    match = re.search(pattern, meta_data, re.IGNORECASE)
                    if match:
                        analysis_result[key] = int(match.group(1))
            
            # Calculate distraction score (lower is better)
...
- Lane Violations: {camera_metrics.get('lane_violations', 0)}
- Traffic Signal Violations: {camera_metrics.get('traffic_violations', 0)}
- Distraction Penalty Score: {camera_metrics.get('total_distraction_score', 0)}
    ...
    try:
        ...
        insights_response = {
            ...
            "obd_data_summary": {
                ...
                "speed_violation_details": obd_metrics.get("speed_violations", [])[:10],  # Top 10 violations
                "harsh_event_details": obd_metrics.get("harsh_events", [])[:10]  # Top 10 events
            },
            "camera_data_summary": {
                ...
                "distraction_score": camera_metrics.get("total_distraction_score", 0)
            },
            "context_breakdown": {
                "urban_harsh_accelerations": obd_metrics.get("urban_harsh_accelerations", 0),
                "highway_harsh_accelerations": obd_metrics.get("highway_harsh_accelerations", 0),
                "urban_harsh_brakes": obd_metrics.get("urban_harsh_brakes", 0),
                "highway_harsh_brakes": obd_metrics.get("highway_harsh_brakes", 0)
            },
            "status": "success"
        }
...
def analyze_driving_behavior():
    ...
    try:
        ...
        '''response = {
            ...
            "detailed_analysis": {
                "vehicle_performance": {
                    ...
                    "distance_km": round(obd_metrics.get('distance_traveled', 0), 2),
                    "total_data_points": obd_metrics.get('total_records', 0)
                },
                "traffic_compliance": {
                    "speed_violations": obd_metrics.get('speed_violation_count', 0),
                    "speed_violation_details": obd_metrics.get('speed_violations', [])[:3]  # Top 3 violations
                },
                "attention_and_behavior": {
                    "safe_distractions": camera_metrics.get('safe_distractions', 0),
                    "critical_distractions": camera_metrics.get('critical_distractions', 0),
                    "lane_violations": camera_metrics.get('lane_violations', 0),
                    "traffic_signal_violations": camera_metrics.get('traffic_violations', 0),
                    "distraction_score": camera_metrics.get('total_distraction_score', 0)
                }
            },
            "status": "success"
        ...
        '''return jsonify(convert_numpy(llm_response))'''
...
if __name__ == '__main__':
    ...
    print("- POST /insights (ask for trip insights)")
